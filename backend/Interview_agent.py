...
async def entrypoint(ctx: JobContext):
    ...
    allow_interruptions=False,         # 👈 Disable user interruptions completely
    resume_false_interruption=True,   
    min_interruption_duration=1.0,     # 👈 Ignore small noises
...
#This will take about 15-20 minutes. Take your time with your responses and feel free to be detailed. Ready to begin?"""
    ...
    #await session.generate_reply(instructions=greeting, allow_interruptions=False)
    #await session.wait_for_user_speech_end(timeout=90)
    ...
    # --- Ask main question ---
        ...
        await session.generate_reply(instructions=f"Ask this question naturally: {question}", allow_interruptions=False)
    
    # Wait for user's complete answer with timeout protection
        try:
            await asyncio.wait_for(session.wait_for_user_speech_end(), timeout=90)
        except asyncio.TimeoutError:
            print(f"⏳ No user response for Q{i+1}, moving on...")

    # Increment counter early to ensure progress
        state.questions_asked += 1  

    # --- Ask single follow-up ---
        follow_up_prompt = (
        f"Based on the user's last answer to '{question}', "
        "ask one simple follow-up question. Keep it short and specific."
        )
        await session.generate_reply(instructions=follow_up_prompt, allow_interruptions=False)

        try:
            await asyncio.wait_for(session.wait_for_user_speech_end(), timeout=60)
        except asyncio.TimeoutError:
            print(f"⏳ No response to follow-up {i+1}, moving on...")
...
and your interview report will be generated automatically. Have a great day ahead!"""
    ...
    await session.generate_reply(instructions=closing, allow_interruptions=False)
